...
const deleteAnswer = async (req, res) => {
  try {
    ...
    if (answer.author.toString() == req.user._id.toString() || req.user.roles.includes("moderator")) {
      await answer.delete();
      return res.status(200).json({ message: "Answer was deleted" });
    }

    return res.status(401).json({ message: "You are not allowed to delete this answer" });
  } catch (err) {
